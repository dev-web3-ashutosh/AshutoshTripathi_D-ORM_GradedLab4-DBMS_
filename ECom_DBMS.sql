...
2) You are required to develop SQL based programs (Queries) to facilitate the Admin team of the E-Commerce company 
   to retrieve the data in summarized format - The Data Retrieval needs are described below.
/*------------------------------------------------------------------------*/
...
SELECT 
    ...
    COUNT(DISTINCT c.CUS_ID) AS TotalIndividualCustomers
FROM
    customer AS c
        INNER JOIN
...
/*------------------------------------------------------------------------*/
SELECT 
    o.ORD_ID AS OrderID, p.PRO_NAME AS ProductName
FROM
    orders AS o
        INNER JOIN
...
/*------------------------------------------------------------------------*/
/* 
10) Create a stored procedure to display supplier id, name, Rating(Average rating of all the products sold by every supplier)
...
CREATE PROCEDURE sp_GetSupplierRating()
BEGIN
    SELECT 
        s.SUPP_ID,
        s.SUPP_NAME,
    ...
    FROM 
        rating as r
			JOIN
		orders as o on r.ORD_ID=o.ORD_ID
			JOIN
		supplier_pricing as sp on o.PRICING_ID=sp.PRICING_ID
			JOIN
		supplier as s on sp.SUPP_ID=s.SUPP_ID
	GROUP BY SUPP_ID
    ORDER BY SUPP_ID;
END //
